    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
        ...
        <div className="relative order-1" ref={leftBoxRef}>
          <span className="text-sm font-semibold text-red-600 mb-2 absolute -top-4 left-6 bg-white px-4 py-1 shadow-sm border border-red-100 rounded-full z-10">Expert Input</span>
          {isLoadingData ? (
          ...
          {!isLoadingData && (
            <div className="absolute bottom-5 right-6">
              <span className={`text-sm px-4 py-2 rounded-full font-medium shadow-sm ${
                ...
                {value?.length || 0}/{section.minLength} characters
              </span>
            </div>
          )}
        </div>
            ...
            {aiRecommendationsAvailable && section.aiRecommendation ? (
              <>
                ...
                <div className="flex justify-end mt-6 relative z-10">
                  ...
                  >
                    {value === section.aiRecommendation ? 'Restore My Text' : 'Use AI Recommendation'}
                  </Button>
                </div>
              </>
            ) : (
              <div className="h-full flex flex-col items-center justify-center text-center relative z-10">
                ...
                <p className="text-gray-700 font-medium mb-3 text-lg">AI Recommendations Not Generated</p>
                <p className="text-gray-600 text-sm max-w-xs mx-auto">Please upload PDF file at the top of the page and wait for analysis to complete</p>
              </div>
            )}
          </div>
        </div>
